        ...
        $.ajax({
            ...
            success: function (json) {
                ...
                if(json.success) {
                    ...
                    userData = json.data;
                    $("#bar2").focus();
                    $.alert("本页保存完成【还请继续填报信息】", "提示");
                    initUserResult();
                } else {
                    $.alert("本页保存完成异常:[" + json.msg + "]", "警告");
                }
            }
        });
    });
    ...
    $("#save-case-touching").click(function() {
        ...
        $.ajax({
            ...
            success: function (json) {
                ...
                if(json.success) {
                    // 下一步
                    selectUserInfoAll(userData["userid"], userData["name"]);
                    ...
                    getGroupCount();
                    $.alert("<span class='text-danger'>【请向下滑动屏幕】</span>，再次检查您填写信息是否正确。<br>今天提交完成!<br>如有修改，可再次提交!", "感谢支持");
                } else {
