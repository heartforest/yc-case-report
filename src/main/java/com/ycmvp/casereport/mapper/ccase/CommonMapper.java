...
public interface CommonMapper {
    ...
    @Select("SELECT GROUPNAME, count(*) org_cnt " +
            ...
            "    LEFT JOIN TAB_CASE_USER tcu ON tu.USERID = tcu.USERID " +
            "WHERE USER_STATUS = '正常' AND tcu.CASE_DATE = #{caseDate} " +
            "AND tcu.EXAMINE_NAME is not null AND tcu.EXAMINE_NAME <> '' " +
            "GROUP BY GROUPNAME ")
    List<Map> findGroupCountExamine(@Param("caseDate") LocalDate caseDate);
    ...
    @Select("SELECT GROUPNAME, tu.name, tcu.case_date, tu.userid " +
            "FROM TAB_USER tu " +
